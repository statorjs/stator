...
- **`attr` on element** — `setAttribute(name, value)` on the element; **`value: null` removes the attribute** (`removeAttribute`). Null-removal is how boolean attributes (`disabled`, `checked`, `open`, `inert`, …) toggle by presence: server-side, an attr binding whose read() yields `false`/`null`/`undefined` renders the attribute absent and patches `null`; `true` renders present-and-empty (`disabled=""`) and patches `''`. Caveat: `checked`/`value`/`selected` as *attributes* set defaults only — a user-dirtied form control ignores them, which is the intended pre-fill semantics: the control owns its draft; commits are typed events (the forms pattern).
