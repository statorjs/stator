...
- ~~**The reload key.**~~ **Resolved 2026-08-22:** `EffectMeta.session` — `{ id, claims<T>() }` — is set by the server host for session machines on every effect (`runSessionEffect` loads the claims under `CLAIMS_KEY` once per run and passes them through `EffectInvocation.run(signal, session)`; the engine merges opaquely). An entry effect on `initial` reloads by identity with no client round trip; app machines and islands get `undefined`. Note the rhythm: effects run after commit, so a fresh machine renders `loading` and the completion patches in (live routes) or shows on the next request — the ordinary entry-effect contract, not a flash the client must paper over. Tests: `tests/effect-session.test.ts`; docs: persistence guide idiom rewritten on it, `reference/machine` § meta.session.
...
- ~~**Externalizing siblings vs inlining.**~~ **Resolved 2026-08-22 — siblings are bundled.** Evidence from `weather`: `weather.ts` imports `DEFAULT_CLOCK`/`DEFAULT_UNITS` as *values* from `settings.ts` and mirrors them into its own context; with siblings external, editing the default reset `SettingsMachine` but not `WeatherMachine`, and `PanelsMachine` (which formats from Weather's mirror) kept showing the old format — a false negative. Inlining over-resets (an identity-only importer resets when its source changes), which the principle permits. A later refinement could externalize a sibling only when the importer takes nothing but its default export; not worth it until an app logs the over-reset as a paper cut.
...
- Dev: native server uses `metafile.inputs` for machines' affected set and rebuilds the store when any machine's inputs changed; both dev servers log changed machines on rebuild. **Landed 2026-08-22:** the registry keeps each def's `inputs` (`codeInputsOf`, exported from `/server` so the Vite dev server reads its runtime instance); both dev servers rebuild the store when an edited file is in any machine's closure (not only under `machines/`) and log `machine code changed — their sessions start fresh on the next request` with the names, or `… no machine code changed — sessions carry over`. Found while landing: the closure must be derived from the resolved import graph (`metafile.inputs`), not `outputs[*].inputs`, which lists only byte-contributing modules — an imported constant the minifier inlines contributes no bytes yet is code the machine depends on. End-to-end test in `dev-native.test.ts`: a session increments, `lib/counter-step.ts` is edited, the same cookie renders fresh and the next increment uses the new step.
