...
- **Measured cost.** Confirm single-invocation build hashing on `weather` (5 machines) and on the largest example, and a dev rebuild's per-machine cost; write the numbers here and wire the build/rebuild reporting the scale plan depends on.
...
- Build: `buildApp` hashes every machine (failing on an unbundleable closure) and writes `manifest.machines`; `stator build` prints the diff against the previous manifest when present and the hashing time.
