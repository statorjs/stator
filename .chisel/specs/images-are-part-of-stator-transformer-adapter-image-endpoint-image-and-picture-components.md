...
## Pre-release review adjudications (2026-08-28)

An external-style review of the branch (verdict: "the road is right"; peer comparison affirmed extension-as-format over Accept/Vary negotiation, type-level dimensions, and the art-direction matrix as ahead of Next/Astro/Nuxt) surfaced two must-fix findings before the URL grammar froze, both taken:

1. **Widths drift → ambient config.** The components' default widths were a copy of the endpoint's, so `images: { widths: [...] }` plus a bare `<Image>` emitted `?w=` values the endpoint 400s — broken responsive images visible only in production. Fixed structurally: `renderRoute` carries `{ widths, aspectRatios }` from the resolved config onto the render state, and `<Image>`/`<Picture>`/`getImage` read it ambiently (explicit prop > configured allowlist > shipped default, which is now imported from the endpoint module, not copied). Component markup can no longer drift from what the endpoint accepts.
2. **Crop bound reworked: aspect allowlist replaces `h ∈ widths`.** The first cut's bound silently forbade every photographic aspect (16:9 had zero valid candidates; 3:2 one; 4:3 one) — and the dogfood used square, the only aspect that always worked, so it never stressed this. New rule: `?w=&h=` is valid iff `w` is allowlisted AND `h === round(w / a)` for an allowlisted aspect (`images.aspectRatios`, default square + 4:3, 3:2, 16:9 + portrait duals). Variant space stays finite at |widths| × |aspectRatios|. A `<Picture>` source whose ratio the endpoint would reject now **throws** at render — never a silent drop.

Also taken: `aspect` → `aspectRatio` naming unified across `ImageDims`/`PictureSource`/config before freezing; in-flight dedup on cold-cache variant generation (stampede guard); dotfile path segments rejected by the endpoint (`.variants/` unreachable — no variants-of-variants); malformed percent-encoding is a 404, not a 500; the stale `transform` signature in the server reference fixed.

**Deferred, recorded as the expectation-gap list** (each additive): a `?q=` quality allowlist + prop; blur-up placeholders (a write-time thumbhash from `probe` — the dimensions-are-data doctrine applied again); a CDN/URL-builder seam (`images.resolveUrl` — the transformer swaps the *engine* behind this origin, it cannot retarget delivery to a Cloudinary-style host; a remote service today means implementing `ImageTransformer` behind your origin, and the spec should not oversell that); making `format: 'webp'` the `<Image>` default (Astro's posture; support is universal — the extension stays honest either way); density descriptors for fixed-size images; multiple mounts (committed content images vs runtime uploads); smarter `sizes` defaults.

**Cache-durability additions to the list** (2026-08-29, from the tonysull.co dogfood — the caches currently survive deploys only by colocation with the media dir on the mounted volume):

1. **Cache epoch stamping** — variants keyed under `.variants/<epoch>/…` where epoch = hash(transformer identity + sharp version + width/aspect allowlists); boot deletes dead epochs. The variant cache keys on original mtime alone today, so a sharp upgrade or allowlist change silently serves stale-encoder output forever (and the mtime ETag keeps it in CDNs). One change fixes the invalidation class and provides free GC for config churn. The principle: a disk cache must carry its generator's identity in its key (the dogfood's OG cache already does — `OG_TEMPLATE_VERSION` in the hash). Strongest candidate to ship in a 2.9.x patch/minor.
2. **`images.cacheDir` override** — decouple cache location from originals for the committed-in-repo-originals case (where `.variants` otherwise lands in the build context). Gated on the `media/` convention dir landing (next major).
3. **`buildApp` excludes `images.dir` from the dist copy automatically** — the build reads the config, so it can skip the media tree instead of relying on per-app `.dockerignore` discipline; the dogfood hit the baked-into-dist footgun on day one. Arguably graduates from this list to a straight fix.
