...
One more defect in the shipped shape, visible only under load: the deadline's clock starts when a write is *issued*, not when it reaches the socket. A burst of writes to one healthy but slow connection queues inside the stream writer, and the later ones can exceed the deadline while waiting their turn, dropping a connection that was draining fine. The write queue below fixes this by construction.
...
Step 1 needs no measurement to justify: a request must not wait on someone else's socket, and that is a structural property rather than a tuning question. Steps 2 and 3 wait for numbers, and the incident that opened this spec is not one of them.
