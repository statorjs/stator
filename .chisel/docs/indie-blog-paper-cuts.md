...
## 12. Art direction: the hard mode of <picture>, dogfooded (2026-08-28, build)

Tony's call: format fallback is `<picture>`'s trivial mode; art direction is why it's hard. Building it surfaced the real shape: (1) the sources matrix is media × format and MUST include the stored format per media condition — miss that row and a browser without modern-format support silently falls past the media condition to the uncropped fallback (the classic art-direction bug in hand-written picture markup); (2) crops under a bounded variant space constrain aspect choice — both crop axes come from the width allowlist, so `aspect: 1` always works and other ratios only produce candidates where `round(w/aspect)` lands on the list (skipped per width, not an error); (3) `&` in srcset URLs HTML-escapes to `&amp;` in attributes — correct markup that briefly looked like a bug in tests. The feed card now serves square crops under `(max-width: 30rem)` and the full frame elsewhere.

## 13. Form bounce loses fields — the three-tier answer, and fresh evidence for refused-dispatch reasons (2026-08-28, dogfood)

Tony hit it uploading a photo with a blank alt: the validation bounce navigates (303 → new document), so the typed post vanished. The layered resolution: (1) shipped here — server-canonical draft RECOVERY: the handler stashes bounced fields in the owner session machine, the form pre-fills from it (pre-fill-is-a-server-rendered-attribute doctrine), a successful publish clears it; the file itself can't round-trip (browsers forbid pre-filling file inputs) so the label says "re-pick your file". (2) The client tier — shared shape rules failing before submit — is the registration two-tier pattern and belongs to the Stage B editor island (lib/media's rules need an fs-free extraction to run client-side). (3) The no-navigation shape Tony independently proposed — form posts as a typed event, guard refuses, error PATCHES in, fields never reset — is Stator's native form for non-file forms and is blocked on the parked **refused-dispatch reasons** work (the wire returns `committed` with no why); files can't ride the event wire at all, so multipart forms keep the POST + recovery shape regardless. His question is the third independent motivation for reasons-on-refusal.

## 14. read() inside <textarea> rendered its slot wrapper as literal text (2026-08-28, dogfood → framework gate)

Tony saw `<span data-slot="s1:btrue:s1"></span>` as the textarea's pre-fill: text-position `read()` wraps its value in a live-slot span, and RCDATA elements (`textarea`, `title`) treat children as raw text — the wrapper became content. Same disease family as region-markers-in-tables (a framework-injected node in a parser context that can't host it), and the runtime-correctness category by the book: silently wrong on a natural template. Fixes: the example pre-fills via STATIC selector property access (`{owner.draft?.content}` — correct anyway, the draft only changes across navigations; liveness was pointless), and the compiler gains an RCDATA placement gate — `read`/`each`/`when`/`match`/`defer` inside `<textarea>`/`<title>` children are compile errors with the static-value fix in the message, while attribute-position bindings on those elements stay legal. Also caught: the original test passed against the broken output (the span still *contained* the text) — the assertion now requires the textarea body to be raw and slot-free. Untested residue: the runtime `html``` path (function components) has no equivalent tag-context guard; noted, exotic.
