...
Stator generates per-component type declarations into `.stator/types/` so your editor and `tsc` can type-check component props. Two settings make that work — `rootDirs` (so the generated declarations resolve alongside your source) and `allowImportingTsExtensions` (the code imports machines with explicit `.ts` paths):
