...
Stator generates per-component type declarations into `.stator/types/` so your editor and `tsc` can type-check component props. Three settings make that work — `rootDirs` (so the generated declarations resolve alongside your source), `allowImportingTsExtensions` (the code imports machines with explicit `.ts` paths), and including `**/*.stator` (so the [editor language server](/introduction/installation/) treats your components with these options rather than defaults):
