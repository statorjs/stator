...
- **`STATOR_SSE_WRITE_TIMEOUT_MS`** — how long one write to one live page may take before that connection is dropped, default 5000. A half-open socket (a sleeping laptop, a dropped NAT mapping) never settles its writes once its buffer is full, and a dropped connection costs a reconnect and a resync, which is cheap.
- **`STATOR_SSE_QUEUE_MAX_BYTES`** — how much a live page may have waiting behind its in-flight write before that connection is dropped, default 1048576. Only what is waiting counts, so a single large write never trips it.
