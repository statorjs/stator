...
This walks the smallest complete loop in Stator: a machine holds state, a route reads it, an event changes it, and the page updates. It assumes you've [scaffolded a project](/introduction/installation/) (`pnpm create stator`) — or wired the same `machines/`, `routes/`, and `stator` CLI scripts by hand — and can run `stator dev`.
