...
**Not the tool:** offline-first or local-first apps, canvas-heavy interactive surfaces where nearly everything is client state, static content sites, or systems that must run multi-replica from day one (horizontal scaling is designed but not shipped).

The longer version, including the tradeoffs you accept, is in [Why Stator?](/introduction/why-stator/)
...
The button POSTs a typed event. `CartMachine` transitions. The `<p>` gets a text patch. That's the whole model — the [tutorial](/tutorial/01-setup/) builds a real app on it in nine short chapters.
